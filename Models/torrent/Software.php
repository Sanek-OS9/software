...
abstract class Software{
    ...
    {
        $items = R::findAll('torrent', '`platform` = ? LIMIT ?', [$platform, $limit]);
        $files = [];
    ...
    {
        $items = R::findAll('torrent', '`runame` LIKE ? LIMIT ?', ['%' . $search . '%', $limit]);
        $files = [];
        foreach ($items as $file) {
    ...
    static function getCountPlatform():array
    {
        $array = [];
        $array['all']['files'] = 0;
        $counts = R::getAll("SELECT COUNT(`platform`) AS 'count', `platform` FROM `torrent` GROUP BY `platform` ORDER BY `count` DESC");
